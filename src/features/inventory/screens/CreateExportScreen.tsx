 ...
 * Screen for creating inventory export papers
 */
...
const CreateExportScreen: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    Alert.alert(
      'Xác nhận xuất kho',
      `Bạn có chắc chắn muốn xuất ${totalItems} sản phẩm khỏi kho?`,
      [
        { text: 'Hủy', style: 'cancel' },
        {
          text: 'Xác nhận',
          onPress: async () => {
            try {
              setSubmitting(true);
              const code = 'XK' + Date.now().toString().slice(-8);
              ...
              const response = await inventoryService.exportInventory(requestData);

              if (response.success) {
                Alert.alert('Thành công', 'Xuất kho thành công', [
                  {
                    text: 'OK',
                ...
                Alert.alert('Lỗi', response.message || 'Không thể xuất kho');
              }
            } catch (error) {
              console.error('Error exporting:', error);
              Alert.alert('Lỗi', 'Có lỗi xảy ra khi xuất kho');
            } finally {
              setSubmitting(false);
            }
          },
  ...
  const renderProductItem = ({ item }: { item: ExportItem }) => (
    <ProductItemCard
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScreenHeader
        title={t('export.title', 'Tạo phiếu xuất kho')}
        showBack
