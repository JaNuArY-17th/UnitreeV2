    ...
    try {
      ...
      if (response.success && apiResponse && apiResponse.data) {
        ...
        if (access_token && refresh_token) {
          ...
          // Fetch and cache user data after successful OTP verification
          let determinedBankType: 'STORE' | 'USER' | undefined;
          try {
            console.log('🔄 [OTPVerify] Fetching user data to cache...');
            const { userService } = await import('../services/userService');
            const myDataResponse = await userService.getMyData();

            if (myDataResponse.success && myDataResponse.data) {
              console.log('✅ [OTPVerify] User data fetched successfully');
              
              // Determine bank type using is_shop field (no login tab context in OTP flow)
              const userIsShop = myDataResponse.data.is_shop;
              const accountType = myDataResponse.data.account_type;
              
              if (userIsShop) {
                determinedBankType = 'STORE';
                console.log('🏪 [OTPVerify] Bank type: STORE (is_shop = true)');
              } else if (accountType === 'STORE') {
                determinedBankType = 'STORE';
                console.log('🏪 [OTPVerify] Bank type: STORE (account_type = STORE)');
              } else if (accountType === 'USER') {
                determinedBankType = 'USER';
                console.log('👤 [OTPVerify] Bank type: USER (account_type = USER)');
              } else {
                console.log('⚠️ [OTPVerify] Bank type could not be determined from API data');
              }
              
              // Update bank type manager only if determined
              if (determinedBankType) {
                const { bankTypeManager } = await import('@/features/deposit/utils/bankTypeManager');
                bankTypeManager.setBankType(determinedBankType as any);
              }
              console.log(`🏦 [OTPVerify] Bank type manager updated: ${determinedBankType}`);
              
            } else {
              console.warn('⚠️ [OTPVerify] User data API returned unsuccessful response');
            }
          } catch (myDataError) {
            console.warn('❌ [OTPVerify] Unable to fetch user data:', myDataError);
          }

          // Automatically fetch and cache bank account data after successful OTP verification
          try {
            console.log(`🏦 [OTPVerify] Fetching bank account data for ${determinedBankType} type...`);
            const { bankService } = await import('@/features/deposit/services/bankService');
            const { BANK_QUERY_KEYS } = await import('@/features/deposit/hooks/useBankAccount');
            
            const bankAccountResponse = await bankService.getMyBankAccount({ bankType: determinedBankType });
            
            if (bankAccountResponse.success && bankAccountResponse.data) {
              console.log('✅ [OTPVerify] Bank account data fetched successfully');
              // Note: We can't directly access queryClient here, but the data will be available for the next query
              
              // Also try to fetch linked banks
              try {
                await bankService.getLinkedBanks();
                console.log('✅ [OTPVerify] Linked banks data fetched successfully');
              } catch (linkedError) {
                console.warn('❌ [OTPVerify] Unable to fetch linked banks:', linkedError);
              }
            } else {
              console.warn('⚠️ [OTPVerify] Bank account API returned unsuccessful response');
            }
          } catch (bankError) {
            console.warn('❌ [OTPVerify] Unable to fetch bank account data:', bankError);
          }
        }
      }
